...
class CaptioningSolver(object):
    ...
    def __init__(self, model, data, **kwargs):
        ...
        self.print_bleu_score = kwargs.pop('print_bleu_score', False)
    ...
    def check_accuracy(self, X, y, num_samples=None, batch_size=100):
        ...
        for i in range(num_batches):
            ...
            gt_captions = y[start:end]
            generated_captions = self.model.sample(X[start:end], max_length = 30)
            print(gt_captions.shape)
            print(generated_captions.shape)
            print(X[start:end].shape)
            for gt_caption, generated_caption in zip(gt_captions, generated_captions):
                bleu += self.BLEU_score(gt_caption, generated_caption)
                num_iters += 1

        return bleu / num_iters
    ...
    def train(self):
        ...
        for t in range(num_iterations):
            ...
            '''if self.print_bleu_score:
                bleu_score = self.check_accuracy(X = features, y = captions, num_samples = None, batch_size = 100)
                if self.verbose and t % self.print_every == 0:
                    print("Batch Avg. BLEU Score: {:.4f}".format(bleu_score))'''
